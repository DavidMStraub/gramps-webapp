...
class TestSQLAuth(unittest.TestCase):
    def test_pwhash(self):
        sqlauth = auth.SQLAuth("sqlite://", logging=False)
        pwhash = sqlauth.hash_password("Xälz")
        test_correct = sqlauth.verify_password("Xälz", pwhash)
        test_wrong = sqlauth.verify_password("Marmelade", pwhash)
    ...
    def test_add_user(self):
        ...
        with self.assertRaisesRegex(ValueError, r".* already exists"):
            ...
            sqlauth.add_user("test_user", "123", fullname="Test User")
        user = sqlauth.session.query(auth.User).filter_by(name="test_user").scalar()
    ...
    def test_change_names(self):
        ...
        sqlauth.add_user("test_user", "123", fullname="Test User")
        guid = sqlauth.get_guid("test_user")
        sqlauth.modify_user(guid, name="test_2", fullname="Test 2")
        user = sqlauth.session.query(auth.User).filter_by(id=guid).scalar()
    ...
    def test_change_pw(self):
        ...
        sqlauth.add_user("test_user", "123", fullname="Test User")
        guid = sqlauth.get_guid("test_user")
        sqlauth.modify_user(guid, password="1234")
